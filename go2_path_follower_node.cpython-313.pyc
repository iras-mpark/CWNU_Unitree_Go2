...
class PotentialAStarPlannerNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("scan_topic", "/utlidar/accumulated_cloud")
        ...
        self.create_subscription(PointCloud2, self.scan_topic, self._scan_cb, 5)
        ...
        self.get_logger().info(
            f"Potential A* planner ready: scan={self.scan_topic}, target={self.target_topic}, "
            f"grid={self.width}x{self.height}@{self.resolution:.2f}m, "
            f"obstacle_z=[{float(self.get_parameter("obstacle_z_min_m").value):.2f},"
            f" {float(self.get_parameter("obstacle_z_max_m").value):.2f}]m"
        )

    # ---------------------------------------------------------------- callbacks
    def _scan_cb(self, cloud: PointCloud2) -> None:
